...
func (a *Api) onTrackGender(w http.ResponseWriter, req *http.Request) {
	switch req.Method {
	case http.MethodGet:
		a.getOnTrackGender(w, req)
	case http.MethodPost:
		a.setOnTrackGender(w, req)
	}
}

func (a *Api) getOnTrackGender(w http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
...
func (a *Api) setOnTrackGender(w http.ResponseWriter, req *http.Request) {
	ctx := req.Context()
	s := lo.Must(work.GetSession(ctx))
	...
	err = a.onTrackService.SetOnTrackGenderStatus(ctx, status)
	if err != nil {
		http.Error(w, fmt.Sprintf("cannot update on track status: %s", err), http.StatusInternalServerError)
		return
	}
